...
> **Project note (Do You Know) — read before following the "Git Checkpoints" section below:** this project currently has no test runner configured (no Jest/Vitest, no `test` script in `package.json`, no test files) and the `node scripts/setup-package-manager.js --detect` helper this skill assumes does not exist here — Step 0 has nothing to detect yet. More importantly, the "Git Checkpoints" section instructs committing automatically at each RED/GREEN/refactor stage; **that conflicts with this project's standing rule to never commit unless the user explicitly asks.** Do not auto-commit checkpoints for this project — surface the same RED/GREEN evidence in chat instead, and only commit when asked to, as usual. Everything else (test structure, patterns, mocking approach) is still useful reference once/if a test runner is added; adapt the Supabase mock example below (it's already correct) and skip the Next.js `NextRequest`/API-route example (no custom backend here).
...
### 4. Git Checkpoints (see project note above — do not auto-commit for this project)
...
1. Check `package.json` `scripts.test` and for a Jest/Vitest config file. (This project has neither yet — see project note above.)
2. **Distinguish the package manager from the test runner — they are not the same.** A project can use one tool to install dependencies yet run a different one for tests. Inspect `package.json` `scripts.test` and the test files:
   - `scripts.test` invokes `jest` / `vitest` -> run through the project's package manager (`npm test`, `pnpm test`, `yarn test`, or `bun run test`).
   - `scripts.test` is `bun test`, or test files `import { test, expect } from "bun:test"` -> use Bun's native runner (`bun test`), which is a Jest-like but distinct API.

Runner command matrix (once a runner exists):
...
Recommended commit message format (only when the user has asked for a commit — see project note above):
...
### Unit Test Pattern (Jest/Vitest + React Native Testing Library)
```typescript
...
describe('FriendRow', () => {
  it('renders the friend name', () => {
    render(<FriendRow name="Lena" avatarEmoji="🦊" streak={3} onPress={() => {}} />)
    expect(screen.getByText('Lena')).toBeTruthy()
  })

  it('calls onPress when tapped', () => {
    const onPress = jest.fn()
    render(<FriendRow name="Lena" avatarEmoji="🦊" streak={3} onPress={onPress} />)
    fireEvent.press(screen.getByText('Lena'))
    expect(onPress).toHaveBeenCalledTimes(1)
  })
})
```

(Adapted from the upstream `@testing-library/react` + DOM example to React Native's `@testing-library/react-native`, since this project has no DOM.)

### Bun Native Test Pattern (`bun:test`)

When the project uses Bun's built-in runner (see Step 0), import from `bun:test` and run with `bun test` — not `bun run test`. The API is Jest-like, so `describe` / `it` / `expect` and most matchers carry over.
...
### E2E Test Pattern (Playwright, for the web export)

This project already uses Playwright manually for one-off testing during development (see conversation history) — this pattern is how to turn that into a real, repeatable suite.

```typescript
import { test, expect } from '@playwright/test'

test('friend appears on Home immediately after being accepted', async ({ page }) => {
  await page.goto('/')
  await expect(page.getByText('Freunde')).toBeVisible()
  // ... drive the add-friend / accept-request flow, then assert the new
  // friend's row renders on Home without a page reload.
})
```
...
├── components/
│   ├── friend-row.tsx
│   └── friend-row.test.tsx          # Unit tests
├── state/
│   ├── appStore.ts
│   └── appStore.test.ts             # Store logic tests
└── e2e/
    └── friends.spec.ts               # E2E tests (Playwright)
```

(Adapted to this project's actual `src/` layout — see `AGENTS.md`/`CLAUDE.md` for the full structure.)
...
Origin: imported from github.com/affaan-m/ecc (skills/tdd-workflow/SKILL.md), commit bf70150 — trimmed the Next.js API-route example and Redis/OpenAI mocks (not part of this stack), adapted the component test example to React Native Testing Library, and added the project notes above.

**Remember**: Tests are not optional. They are the safety net that enables confident refactoring, rapid development, and production reliability. (For this project: apply this once tests are actually requested, not proactively — see project note.)
