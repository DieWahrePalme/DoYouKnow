...
> **Project note (Do You Know):** this project develops directly on the feature branch named by the harness (currently `claude/social-game-swipe-mvp-0dukax`) rather than opening a fresh branch per change — that convention, and the stricter git-safety rules already in force for this session (never force-push without asking, prefer a new commit over `--amend`, always confirm before destructive operations), take precedence over the generic advice above where they conflict.

Origin: imported from github.com/affaan-m/ecc (skills/git-workflow/SKILL.md), commit bf70150
