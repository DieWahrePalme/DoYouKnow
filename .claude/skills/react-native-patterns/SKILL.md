...
> **Project note (Do You Know):** this project already uses Expo Router + `StyleSheet` (not NativeWind) + Zustand for everything, including server data — Supabase reads/writes happen directly inside Zustand store actions (see `src/state/appStore.ts`), not through TanStack Query. The "server cache vs client store" separation below is good general guidance and worth considering for a future refactor, but it does not describe how this codebase works today — do not "fix" existing stores to match this skill unless asked. The route-param-validation, list-virtualization, secure-storage, and anti-patterns sections apply as-is.

Practical patterns for building production React Native apps with Expo. Covers navigation, state, data fetching, lists, styling, and native APIs. Pairs with the `.claude/rules/ecc/react-native/` ruleset: rules say *what* to enforce, this skill shows *how*.
...
Origin: imported from github.com/affaan-m/ecc (skills/react-native-patterns/SKILL.md), commit bf70150
