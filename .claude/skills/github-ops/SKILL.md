---
name: github-ops
description: GitHub repository operations, automation, and management. Issue triage, PR management, CI/CD operations, release management, and security monitoring. Use when the user wants to manage GitHub issues, PRs, CI status, releases, contributors, stale items, or any GitHub operational task beyond simple git commands.
metadata:
  origin: ECC
---
...
> **Project note (Do You Know):** commands below are written for the `gh` CLI. In this project's remote Claude Code environment, the equivalent operations go through the `mcp__github__*` MCP tools instead (e.g. `mcp__github__actions_list`/`actions_get` instead of `gh run list`/`gh run view`, `mcp__github__pull_request_read`/`pull_request_review_write` instead of `gh pr view`/`gh pr review`). If you're running this project from a local Claude Code CLI with `gh` installed, the commands below work as written.
...
- **gh CLI** for all GitHub API operations (see project note above for this environment's equivalent)
...
Origin: imported from github.com/affaan-m/ecc (skills/github-ops/SKILL.md), commit bf70150. The upstream file also links a release-checklist reference specific to the ECC project's own npm/tag release process — dropped here as not applicable to this project.
