...
> This project already uses Expo Router (see `src/app/`) and Zustand for state (see `src/state/`) — the "State Management" and "Data Fetching" sections below describe a TanStack-Query-based split this project does not currently follow (Supabase calls happen directly inside Zustand actions, per `src/state/appStore.ts`). Read it as a possible future direction, not a required change.
...
- Validate API responses with Zod at the boundary; infer types from the schema.
...
Origin: imported from github.com/affaan-m/ecc (rules/react-native/patterns.md), commit bf70150
