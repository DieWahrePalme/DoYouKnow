...
- Use `FlatList`/`SectionList`, or `FlashList` (Shopify) for large or heterogeneous lists. This project's Home screen already uses `FlatList` (see `src/app/(tabs)/index.tsx`).
...
- Build on the **New Architecture** (Fabric + TurboModules). It is the default in recent Expo SDKs (opt-out still available on SDK 53–54) and is mandatory — cannot be disabled — from SDK 55+. This project is on Expo SDK 57 (see `AGENTS.md`), so New Architecture is non-negotiable. Verify every native dependency is New-Arch compatible before shipping.
...
Origin: imported from github.com/affaan-m/ecc (rules/react-native/performance.md), commit bf70150
