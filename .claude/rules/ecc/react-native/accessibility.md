...
Origin: imported from github.com/affaan-m/ecc (rules/react-native/accessibility.md), commit bf70150. Directly actionable for this project's current icon-only buttons (e.g. Home's `+`/inbox `TopBar` buttons in `src/app/(tabs)/index.tsx`) — worth a follow-up a11y pass.
